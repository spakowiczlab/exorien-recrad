...
Required data for these plots are saved in a csv file.

# Load data

```{r}
mics <- read.csv("../data/ORIEN_16s-microbes.csv")
key <-  read.csv("../data/16s_sample_matching.csv")
```
...
```{r}
key.form <- key %>%
  mutate(sample.n = as.numeric(substr(sample16s, 9, nchar(sample16s))))

mics.selgen <- mics %>%
  filter(genus %in% c("g__Fusobacterium", "g__Streptococcus", "g__Lactobacillus")) %>%
  group_by(sample, genus) %>%
  summarise(counts = sum(counts)) %>%
  rename("sample.n" = "sample") %>%
  left_join(key.form) %>%
  filter(grepl("Primary", Specimen.Type)) %>%
  inner_join(orien.allvars) %>%
  distinct() %>%
  filter(sample.n != 30299) 

write.csv(mics.selgen, "../data/plotdata_16s-matching.csv", row.names = F)
```

# Plot

```{r}
cutoff <- quantile(mics.selgen$buffa.score,.5)
mics.selgen %>%
  mutate(buffa.status = ifelse(buffa.score < cutoff, "Low", "High"))%>%
  ggplot(aes(x = buffa.status, y = counts)) +
  geom_boxplot() +
  labs(x = "Hypoxia status", y = "Fusobacterium") +
  theme_bw()
ggsave("../figures/boxplot_162-fusobacterium.png")
```
